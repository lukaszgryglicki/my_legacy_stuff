...
void heuristic_count_moves_to_win(int player, int* rmin, int* rways)
{
  for (k=1;k<4;k++)
  {
      if (k == 1)		// 1D case
      {
	  for (j=0;j<4;j++)	// all axis cases
	  {
	      for (c1=0;c1<4;c1++)	// 3 degrees of freedom
	      for (c2=0;c2<4;c2++)
	      for (c3=0;c3<4;c3++)
	      {
		  for (f1=0;f1<4;f1++)	// 1 parameter fixed
		  {
		      if (j==0) vec[f1] = get_at(c1,c2,c3,f1);
		      if (j==1) vec[f1] = get_at(c1,c2,f1,c3);
		      if (j==2) vec[f1] = get_at(c1,f1,c2,c3);
		      if (j==3) vec[f1] = get_at(f1,c1,c2,c3);
		  }
		  cmin=4;
    		  for (i=0;i<4;i++)
       		  {
      ...
      {
	      for (j=0;j<6;j++)
	      for (c1=0;c1<4;c1++)	// 2 degrees of freedom
	      for (c2=0;c2<4;c2++)
	      {
