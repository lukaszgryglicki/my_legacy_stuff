...
int check_config(char* err)
{
	...
	{
	    sprintf(err, "Bad extended mode IDs: %d %d", cfg.myid, cfg.targetid);
	...
	if (cfg.n_sync < 0 || cfg.n_sync > 1000) 
	{
	    sprintf(err, "Sync communication must be in range 0-1000");
	    cvoice_log("GUI: Error: %s", err);
	    return 0;
	}
...
void get_user_input(const char* question, char* ptr, const char* def)
{
    ...
    myret = (MyRetText*)malloc(sizeof(MyRetText));
...
void btn_cfg_display(GtkWidget* _w, gpointer _l)
{
	...
	if (cfg.is_input == 0)
	{
		sprintf(fp, "Output mode only\n");
		strcat(msg, fp);
	}
	else if (cfg.is_input == 1)
	{
		sprintf(fp, "Input mode only\n");
		strcat(msg, fp);
	}

	if (cfg.use_syncer == 0)
	{
		sprintf(fp, "Not using syncer threads\n");
		strcat(msg, fp);
	}
	else
	{
		sprintf(fp, "Using syncer threads\n");
		strcat(msg, fp);
	}
...
void btn_cfg_toggleopt(GtkWidget* _w, gpointer i)
{
    ...
    else if (i == (gpointer)4)	
    {
	cfg.is_input ++;
	if (cfg.is_input == 2) cfg.is_input = -1;
	if (cfg.is_input == -1) strcpy(tmp, "Bidirectional mode");
	else if (cfg.is_input == 0) strcpy(tmp, "Sender only mode");
	else strcpy(tmp, "Reciever only mode");
    }
    else if (i == (gpointer)5)	
    {
	cfg.use_syncer = !cfg.use_syncer;
	if (cfg.use_syncer) strcpy(tmp, "Using syncer threads");
	else strcpy(tmp, "Not using syncer threads");
    }
...
{
    GtkWidget *w, *t, *s, *l, *c, *cl, *bi, *bo, *bd, *cm;
    GtkWidget *bc, *bq, *bx, *bdb, *bwi, *bsa, *bpi, *us;
    ...
    cm = gtk_button_new_with_label( "Comm mode" );
    gtk_table_attach_defaults( GTK_TABLE( t ), cm, 2, 3, 6, 7);

    us = gtk_button_new_with_label( "Use syncer" );
    gtk_table_attach_defaults( GTK_TABLE( t ), us, 0, 1, 7, 8);

    cl = gtk_button_new_with_label( "Close" );
    gtk_table_attach_defaults( GTK_TABLE( t ), cl, 1, 3, 7, 8);
    ...
    g_signal_connect( cm, "clicked", G_CALLBACK(btn_cfg_toggleopt), (gpointer)4);
    g_signal_connect( us, "clicked", G_CALLBACK(btn_cfg_toggleopt), (gpointer)5);
