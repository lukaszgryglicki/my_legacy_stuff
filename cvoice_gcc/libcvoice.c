...
void cv_cfg_clear(cv_cfg* c)
{
	...
	c->cache_size = 70;	/* wide: in 320b packets, 8bit: in 160b packets, 70 = 1,4s (cache in 20ms chunks)	*/
	c->n_sync = 0;		/* sync using oos old method, setting to 0 enables new method of syncer IPC threads, they're mutually exclusive*/
	c->quality = 6;
	c->hz = 16000;
	c->max_oos = 0;		/* 10: out of sync is after max_oos s, 0 to disable */
	c->use_syncer = 1;	/* use syncer threads */
	c->is_input = -1;	/* client will spawn to reader and writer by default */
	...
	c->wide = 1;		/* 16bit audio */
	c->enh_r = 1.2;
	c->enh_w = 1.0;
	c->save = 1;		/* save mspx files */
	c->port = 9000;
	c->pidlog = 1;
...
void cvoice_help(char* name)
{
	printf("%s usage:"
		...
		"\n\t-h\tThis help"
		"\n\t-c\tNetwork cache in 160b (0.02s) packets (70: 1,4s)"
		"\n\t-d\tEnables debug"
		"\n\t-q\tSpeex encoder quality [0-10] (6)"
		"\n\t-T\tNet timeout (-90) 0=off negative enables timeout after connect", name);
	printf(
		"\n\t-I\tInput device (microphone) (/dev/dsp)"
		"\n\t-O\tOutput device (speakers) (/dev/dsp)"
		"\n\t-P\tDo not use PID in log name (off)"
		"\n\t-8\tUse 8bit audio instead of 16bit (off)"
		"\n\t-R\tEnchance input float (1.2)"
		"\n\t-W\tEnchance output float (1.0)"
		"\n\t-S\tTurn off save VoIP data to files (default: save)"
		"\n\t-C\tCompexity 0-10 (10)"
		"\n\t-y\tSync communication after N cycles (N * cache, N=0)"
		"\n\t-H\tSet sample rate in Hz (16000)");
	printf( 
		"\n\t-L\tLoad configuration from file name given"
		"\n\t-Z\tSave configuration to file name given"
		"\n\t-o\tOut of sync after seconds, 0 to disable (0)"
	        "\n\t-r\tRecieve only mode (off)"
		"\n\t-w\tSend only mode (off)"
		"\n\t-0\tDo not use syncer threads/processes on both sides (default: use)"
		"\n");
...
int cvoice_client(int inp, int use_sync)
{
	int pid;
	int sock,n;
...
void cvoice_forked(int listener, int* pid1, int* pid2)
{
	*pid1 = *pid2 = 0;
	...
	cvoice_log("Main startup: server mode: %d, use syncer: %d, input mode: %d", listener, cfg.use_syncer, cfg.is_input);
	...
	{
		if (cfg.is_input != 0) *pid1 = cvoice_client(1, cfg.use_syncer);
		if (cfg.is_input <= 0) *pid2 = cvoice_client(0, cfg.use_syncer);
...
void cvoice_save_config_fp(FILE* fp, cv_cfg* c)
{
	...
	fprintf(fp, "Port=%d\n", c->port);
	fprintf(fp, "IP=%s\n", c->ip);
	fprintf(fp, "IsInput=%d\n", c->is_input);
	fprintf(fp, "UseSyncer=%d\n", c->use_syncer);
...
int cvoice_getopt(int lb, char** par)
{
	...
	*/
	...
	while ((u = getopt(lb,par,"0rwS8as:i:p:f:t:hc:dq:T:I:O:PR:W:C:y:H:L:Z:o:"))!=-1)
	{
		switch (u)
		{
			case 'a': is_server = 1; 		break;
			case 'r': cfg.is_input = 0; 		break;
			case 'w': cfg.is_input = 1; 		break;
			case '0': cfg.use_syncer = 0; 		break;
