 ...
 switch (key)
   {
	...
	case 'w': obsD *= 1.1; break;
	case 's': obsD /= 1.1; break;
	case 'a': eye *= 1.1; break;
	case 'd': eye /= 1.1; break;
...
/* glEnable(GL_CULL_FACE);*/
 ...
 glLoadIdentity();
 gluLookAt(-eye, 0., obsD, 0. , 0., 0., 0., 1., 0.);
...
/* test_inside();*/
 Init();
 if (lb < 5 && lb != 1) fatal("bad parameters count");
 if (lb == 1)
   {
 ...
 printf("stereo? ");
 scanf("%s", hStereo);
 if (hStereo[0] == 'y' || hStereo[0] == 'Y' || hStereo[0] == 't' || hStereo[0] == 'T') bstereo = 1;
 printf("fullscreen? ");
 scanf("%s", hFull);
 if (hFull[0] == 'y' || hFull[0] == 'Y' || hFull[0] == 't' || hFull[0] == 'T') bfull = 1;
/* test_test1();*/
/* test_test2();*/
/* test_test3();*/
/* test_test3_fail();*/
...
/* test_common_edges();*/
 ...
 painter_sort();
 glutInit(&lb, par);
 if (!bstereo) glutInitDisplayMode(GLUT_DOUBLE);
 else          glutInitDisplayMode(GLUT_DOUBLE | GLUT_STEREO);
 glutInitWindowSize(wx, wy);
 glutInitWindowPosition(100, 100);
 glutCreateWindow(par[0]);
 if (!bstereo) glutDisplayFunc(render_scene);
 else glutDisplayFunc(render_scene_stereo);
 ...
 if (want_tex) 
   {
    ...
    else if (lb ==1) init_textures_file("tex0.dat");
