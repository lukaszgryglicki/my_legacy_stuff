 ...
 if ((semid=semget(semk,1,0660|IPC_CREAT|IPC_EXCL))==-1)
   {
    ...
    if (semid == -1) { perror("semget in msem"); return -1; }
   }
 else
   {
    union msemun arg;
...
{
 if (semctl(semid,IPC_RMID,(int)0)==-1) { warn("mdropsem failed for semid %d", semid); return -1; }
 else return 0;
}
...
int mdropsemkey(int semkey)
{
 ...
 if (semctl(semid,IPC_RMID,(int)0)==-1) { warn("semctl in mdropsemkey"); return -1; }
 else return 0;
}
 ...
 if ((queid=msgget(quek,IPC_CREAT|IPC_EXCL|0660))==-1)
   {
    ...
    if (queid == -1) { perror("msgget in mque"); return -1; }
   }
 return queid;
}
 ...
 if ((shmid=shmget(shmk,size,IPC_CREAT|IPC_EXCL|0660))==-1)
   {
    ...
    if (shmid == -1) { perror("shmget in mshm"); return -1; }
   }
 return shmid;
}
