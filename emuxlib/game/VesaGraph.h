...
void DrawFrame(Display* dsp, Window& w, ulong col, GC& gc, long fade, int thin, int sx, int ex, int sy, int ey)
{
#ifdef DEBUG
...
void DrawHalfFrame(Display* dsp, Window& w, ulong col, GC& gc, long fade, int thin, int sx, int ex, int sy, int ey, bool upper)
{
#ifdef DEBUG
...
void XFillA(Display* dsp, Window& w, GC& gc, ulong col, int sx, int sy, int x, int y)
{
#ifdef DEBUG
 ...
 for (int i=0;i<less;i++)
   {
    XDrawLine(dsp, w ,gc, sx+i, sy+i, sx+x-i, sy+i);
    XDrawLine(dsp, w ,gc, sx+i, sy+i, sx+i, sy+y-i);
    XDrawLine(dsp, w ,gc, sx+i, sy+y-i, sx+x-i, sy+y-i);
    XDrawLine(dsp, w ,gc, sx+x-i, sy+i, sx+x-i, sy+y-i);
   }
#ifdef DEBUG
 printf("DEBUG_MGRAPH_H: OK\n");
#endif
