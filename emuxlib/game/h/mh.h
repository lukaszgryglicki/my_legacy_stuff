...
#endif   
 qsortasm((void*)table, NULL, 0, num-1);
}*/
...
void takechar(char& zn)
{
 ...
 scanf("%s", znak);
 sscanf(znak, "%c", &zn);
 if (strlen(znak)>1) printf("podales za dluga nazwe\n");
}
 ...
 while (i<STRING-1)
	{
	 ...
	 if (par[i] == 27) goto st;
	 if (sec != 0xfade) putchar(par[i]);
	 i++;
	 cont:;
	}
 lab:
 if (i == 0) {strcpy(par,"-");putchar('-');}
 ln();
 tcsetattr(fileno(stdin),TCSAFLUSH,&st);
}
...
void takeint(int* par)
{
 ...
 char string[STRING];
 scanf("%s",string);
 if (!sscanf(string,"%d",par)) {*par = 0;printf("Parametr literowy! Zmnieniam na 0\n");}
}
...
void takeint(int& par)
{
 ...
 char string[STRING];
 scanf("%s", string);
 if (!sscanf(string,"%d", &par)) {par = 0; printf("Parametr literowy! Zmieniam na 0\n");}
}
