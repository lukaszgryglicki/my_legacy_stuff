...
*/
//to jest wersja skalujaca kolory i dobrze rysujaca duze przyciski

#include "mh.h"		//Morgothheader Lib (uses another file: mran.h)
#include "EmuXlib.h"
#include "VesaGraph.h"     //Some graph functions (DrawHalfFrames itp.)
...
void Wbox :: ReactOnKey(int key)     //main react function
{
 ...
 else if ((key == Xemul(SHIFT)) || (key == Xemul(INVALID_KEY)))
 {
#ifdef DBG
 char dstr[LONG_STR];
 ...
 else if (key == Xemul(ENTER))                                    //terminate string '\0' when ENTER
  {
#ifdef DBG
 char dstr[LONG_STR];
...
class Wnd                                     //great class Wnd
{
 ...
 inline int RetCx()          const ;
 inline int RetCy()          const ;
 int RetRx()                 const ;
 int RetRy()                 const ;
...
void Wnd ::PrepareWndText(char* txt, int nolets = LEAVE)  //cut char* int parts of char**
{
 ...
 for (int i=0;i<max_len;i++) if (txt[i] == '\n') lines++;
 text = new char*[lines+2];
 if (!text) panic("memory exception in wnd::text");
 for (int i=0;i<lines;i++) {text[i] = new char[max_len+1];if (!text[i]) panic("memory exception in text[]");}
 int j=0;
 int k=0;
...
void Wnd :: EmuXlibModEvent(XEvent& ref)
{
#ifdef DBG
 printf("MODIFYING_EVENT_TYPE_FOR_LIBSVGA_EMULATION!\n");
#endif
 if (knotpressed) { this->event.type = KeyPress; knotpressed=0; }
 else { this->event.type = KeyRelease; knotpressed=1; }
 EmuXMouse(disp, win, gc, this->event, msx, msy);
}
...
void Wnd :: BlackWings() // puts black rectangles onto text!
{
 ...
 for (int i=0;i<lines;i++)
   {                                        /*  color */
     if (text[i]) XDrawStringCol(disp, win, gc, 0, 20, 20 + i*(font_h+3), text[i], strlen(text[i]));
   }
}
...
void Wnd :: DrawBtn(int num)
{
 ...
 XDrawString(disp, win ,gc, btn[num]->RetSx()+btn[num]->RetX()/2-7*hlen,
...
void eXtendedKeyPress(Wnd& wnd, ulong code, void* ptr)
{
...
#endif
 if (!wnd) return;
 if (code == Xemul(36) || code == Xemul(ENTER))
   {
    Button* tmp;
    for (int i=0;i<wnd.RetNumBtns();i++)
      {
 ...
 if ((code == Xemul(23)) || (code == Xemul(100)) || (code == Xemul(104))) wnd.NextDefault();
 if ((code == Xemul(98)) || (code == Xemul(102))) wnd.PrevDefault();
...
void eXtendedKeyPressQ(Wnd& wnd, ulong code, void* ptr)
{
 ...
 if (code == Xemul(24) || code == Xemul(34))
   {
...
void eXtendedKeyRelease(Wnd& wnd, ulong code, void* ptr)
{
 ...
 if (wnd.HaveScroll())
   {
    if (code == Xemul(KEY_UP)) wnd.MoveDefScroll(1);
    if (code == Xemul(KEY_DOWN)) wnd.MoveDefScroll(-1);
    if (code == Xemul(KEY_TAB)) wnd.SelectNextScroll();
   }
...
void wBoxKeyPressFunc(Wnd& wnd, ulong code, void* ptr)
{
...
#endif
 if (!wnd) return;
 if ((code == Xemul(SHIFT_L)) || (code == Xemul(SHIFT_R))) wnd.WboxActivateShift(true);
 int key;
 ...
 wnd.DrawWBox();
 if ((key == Xemul(ENTER)) || (key == Xemul(ESCAPE))) wnd.DoQuit();
}
...
#endif
 if (!wnd) return;
 if ((code == Xemul(SHIFT_L)) || (code == Xemul(SHIFT_R))) wnd.WboxActivateShift(false);
}
