...
inline void __unimp(Wnd& wnd, void* ptr)
{
#ifdef DBG_GAME
 char dstr[LONG_STR];
...
{
#ifdef DBG_GAME
 printf("DBG_info: File: %s, Line: %d, compiled at %s %s\n", __FILE__, __LINE__, __DATE__, __TIME__);
#endif
 if (!plik)
   {
...
void PostRaceFunc(Wnd& wnd, void* ptr)
{
 ...
 btn[0]->SetBtnPos(100, 460);
 btn[1]->SetBtnPos(500, 460);
 char* nam = wnd.CreateExCheckBoxVert("Wybierz rase", tmp, db->ilosc[1], 60, 700, 500, 2, btn);
...
void PostProfFunc(Wnd& wnd, void* ptr)
{
 ...
 btn[0]->SetBtnPos(100, 460);
 btn[1]->SetBtnPos(500, 460);
 char* nam = wnd.CreateExCheckBoxVert("Wybierz profesje", tmp, db->ilosc[2], 60, 700, 500, 2, btn);
...
int LoadCharPicture(Wnd& wnd, void* ptr, bool display = true)
{
 ...
 printf("DBG_info: File: %s, Line: %d, compiled at %s %s\n", __FILE__, __LINE__, __DATE__, __TIME__);
#endif
 gtout("LoadCharPicture from BZ2");
 FILE* plik = NULL;
 char fname[255];
 strcpy(fname, post->pname);
 strcat(fname, ".x");
...
void PostDataFunc(Wnd& wnd, void* far_ptr)
{
 ...
 printf("DBG_info: File: %s, Line: %d, compiled at %s %s\n", __FILE__, __LINE__, __DATE__, __TIME__);
#endif
 gtout("PostDataFunc bridged by void*");
...
void PostAddSkillFunc(Wnd& wnd, void* far_ptr)
{
 ...
 btn[0]->SetBtnSize(100, 24);
 btn[0]->SetBtnPos(200, 460);
 btn[0]->SetVoid((void*)lum);
 char* nam = wnd.CreateExCheckBoxVert("Wybierz umiejetnosc:", tmp, db->ilosc[3], 60, 700, 500, 1, btn);
...
void PostAddSkillCFunc(Wnd& wnd, void* far_ptr)
{
 ...
 nam = wnd.CreateExCheckBoxVert("Wybierz umiejetnosc", tmp, x, 40, 700, 500, 1, btn);
 if (!strcmp(nam,"Wlasna um.")) return;
 int num=0;
 sscanf(nam,"(%d)", &num);
 int poz=1;
...
void __IllegalFunc__(Wnd& wnd, ulong key_code, void* far_ptr)
{
 ...
 printf("DBG_info: File: %s, Line: %d, compiled at %s %s\n", __FILE__, __LINE__, __DATE__, __TIME__);
#endif
 gtout("__IllegalFunc__ !CHEATER!");
 if (!far_ptr) {gtout("in __IllegalFunc__ far_ptr=(void*)NULL, aborted!"); return;}
 ...
 if (key == 'x')
   {
    ...
    gtout("SECURITY_CODE_PASSED ===>> modifying (int*)lum");
...
#endif
 if (!f) return;
 gtout("Alternative <<writing HTML>>");
 fprintf(f,"<center>\n");
...
void Character(Wnd& wnd, int exists = true)
{
 ...
 wybor->CreateButton(10, "Rysunek"        ,50  , 250, __unimp, false, 200);//ok
...
void __IllegalPDFunc__(Wnd& wnd, ulong key_code, void* far_ptr)
{
 ...
 printf("DBG_info: File: %s, Line: %d, compiled at %s %s\n", __FILE__, __LINE__, __DATE__, __TIME__);
#endif
 gtout("__IllegalPDFunc__ !CHEATER!");
 if (!far_ptr) {gtout("in __IllegalPDFunc__ far_ptr=(void*)NULL, aborted!"); return;}
 int key = TransformKey(key_code, 0);
 if (key == 'x')
   {
    int* pd = (int*)far_ptr;
    ...
    sscanf(str,"%d", &code);
    if (code != 0xdead) return;
    gtout("SECURITY_CODE_PASSED ===>> modifying (int*)pd");
    str = wnd.CreateWriteBox("Dodac punktow:");
    code = 0;
    sscanf(str,"%d", &code);
    *pd += code;
...
void AboutFunc(Wnd& wnd, void* ptr)
{
 ...
 wybor->EnableGraph(1);
 wybor->EnableFrames(0);
 wybor->PrepareWndText("XLRPG ver 0.1 pre-alpha\nMorgothDBMA: morgothdbma@o2.pl");
 wybor->SetButtonsNumber(1);
 wybor->CreateButton(0, "Yahooo!",88 , 90, StdOKFunc, true);
...
void Main_Game()
{
 ...
 gtout("MainGame");
  ...
  wnd->SetCaption("XLrpg ver 0.1 by Morgoth");
  ...
  wnd->PrepareWndText("XLRPG wersja pre-alfa\n Milej zabawy!\nnacisnij Q aby zakonczyc");
  wnd->StdTextWrite();
...
#endif
 gtout("main");
 printf("Game: File: %s, Line: %d, compiled at %s %s\n", __FILE__, __LINE__, __DATE__, __TIME__);
 Main_Game();
 gtout("no_SIGSEGV_exit");
 return 0;
}
