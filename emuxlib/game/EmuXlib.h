...
typedef struct _Window
{
 ...
 _Window(int a) { sx=sy=a; lx=ly=0x100; colb=0xffffff; colf=0x0; strcpy(name,"");}
 int sx,sy;
 int lx,ly;
 char name[0x100];
...
{
 Window* w = new Window;
 w->sx = (x1) ? x1 : random( ((vga_getxdim()-x3-20)>0) ? (vga_getxdim()-x3-20) : 1);
 w->sy = (x2) ? x2 : random( ((vga_getydim()-x4-20)>0) ? (vga_getydim()-x4-20) : 1);
 //w->sx = 100;
 //w->sy = 100;
 w->lx = x3;
 w->ly = x4;
 w->colb = p1;
 w->colf = p2;
 strcpy(w->name,"No-name");
 w2 = *w;
 return w2;
}

void DrawTitle(Window& w)
{
