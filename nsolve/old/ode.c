...
void render_scene(void)
{
 ...
 if (follow) glTranslated(-xo, -yo, -zo);
 render_object();
 render_curve_tail();
 render_zero();
 glFlush();
 ...
 if (rank == 2)
  {
   ...
   xo = fpar_f(nEqu, newx, newy, newyp, curr_ypp);
   yo = fpar_f(nEqu+1, newx, newy, newyp, curr_ypp);
   zo = fpar_f(nEqu+2, newx, newy, newyp, curr_ypp);
  }
 if (rank == 1)
  {
   for (i=0;i<nEqu;i++) newyp[i] = fpar_f(i, newx, newy, NULL, NULL);
   xo = fpar_f(nEqu, newx, newy, newyp, NULL);
   yo = fpar_f(nEqu+1, newx, newy, newyp, NULL);
   zo = fpar_f(nEqu+2, newx, newy, newyp, NULL);
  }
 ...
 for (i=0;i<3;i++)
   {
    ...
    switch (i)
      {
	  case 0: if (buff[0] == 'X') k = 1; break;
	  case 1: if (buff[0] == 'Y') k = 1; break;
	  case 2: if (buff[0] == 'Z') k = 1; break;
      }
    if (k != 1) { printf("Bad mapping order for %dth screen coord\n", i+1); return; }
    k = 0;
    while (buff[k] != '=' && k < strlen(buff)) k++;
    k++;
/*    printf("Buff = '%s'\n", buff+k);*/
    fpar_function(i+ne, ne, buff+k);
    if (!fpar_ok(i+ne)) return;
   }
...
void help()
{
 ...
 printf("keys: wsadex ikjlom 123456 q SPACE\n");
}
