...
void set_rows_array(int cur_row, int n_rows, JSAMPARRAY pixel_data, int width, unsigned long** bits, int gs)
{
 JSAMPROW ptr;
...
/*       }*/
    ...
    */
    ...
    while (cinfo.output_scanline < cinfo.output_height)
	   ...
	   set_rows_array(i, l, buffer,*x, *bits, gs);
	   i-=l;
...
void* convert_thread(void* ptr)
{
    ...
    translate_jpeg_file(n, bits, obits, ocx, ocy, ct, nt, udfmode);
#ifndef SILENT
    printf("\nThread %d/%d finished\n", ct+1, nt);
#endif
...
/*	printf("Loaded (%04d/%04d): %s --> (%d x %d)\n", i+1, n, ifn[i], ocx, ocy);*/
    ...
    else
    {
    	...
    	{
        	translate_jpeg_file(n, bits, obits, ocx, ocy, 0, 1, udfmode);
    	}
    }
...
#endif
	return fjpg(par_f[1], par_f[2], n, infiles, atoi(par_f[3]));
