...
void cv_cfg :: clear()
{
	...
	ip = new char[32];
    	myid = targetid = 0;
	cache_size = 20;	// wide: in 320b packets, 8bit: in 160b packets
	debug = 0;
	quality = 1;
	complexity = 10;
	vbr = 1;
...
void cvoice_init_mipc()
{
 	...
 	if (mshmconn(mshmid, &mshm_ptr) == -1) 
	{
    	...
    	if (mshm_ptr) 
    	{
	    memcpy(mshm_ptr, (void*)dummy, 2 * sizeof(double));
    	}
...
void mipc_stats()
{
	...
	if (n_sent > 0 && n_recieved > 0)
	{
		...
		memcpy(mshm_ptr, (void*)bps, 2 * sizeof(double));
	}
#endif
}
...
#else
	else if (signo == SIGUSR1 || signo == SIGINFO)
#endif
	{
...
void cvoice_help(char* name)
{
	printf("%s usage:"
		...
		"\n\t-h\tThis help"
		"\n\t-c\tNetwork cache in 160b (0.02s) packets (20: 0.4s)"
		"\n\t-d\tEnables debug"
		"\n\t-q\tSpeex encoder quality [0-10] (1)"
		"\n\t-T\tNet timeout (-30) 0=off negative enables timeout after connect"
		...
		"\n\t-C\tCompexity 0-10 (10)"
		"\n\t-y\tSync communication after N cycles (50 * cache = 20s)"
...
void my_save(char* buff, short l, int dir, int pid)
{
	FILE* fp;
	char fn[64];
	...
	fwrite((void*)&l, sizeof(short), 1, fp);
...
int my_write(int fd, unsigned char* bstr, int n)
{
	...
	while (nwritten < n)
	{
		...
		if (nw <= 0)
		{
		    cvoice_logd("Partial written: %d/%d (%5.2f%%)\n", 
		    	    nwritten, n, ((float)nwritten * 100.) / (float)n);
		    usleep( 1000 );
		    nerr ++;
		    if (nerr > 100) 
		    {
		    	cvoice_log("Error writing: %d/%d (%5.2f%%) - buffer played truncated", 
		    	    nwritten, n, ((float)nwritten * 100.) / (float)n);
			return nwritten;
		    }
		    continue;
...
int cvoice_nogui_main(int lb, char** par)
{
	int is_server;
	char u;
	// a: server mode
