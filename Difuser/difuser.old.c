...
static int cmp = 0;
static unsigned long long gain_bytes = 0;
...
void add_element(char* elem, int siz, unsigned long ino)
{
 if (!elem) return;
 if (last == NULL)
   {
...
void gain(char* fn)
{
    ...
    if (!f) return;
    fseek(f, 0, SEEK_END);
    gain_bytes += (unsigned long long)ftell(f);
    gain_files ++;
    fclose(f);
...
void logsucc(char* fn, char* dup, int idino)
{
 if (!idino) gain(dup);
 if (!logf) return;
 if (shmode==1) 
...
void logwish(char* fn, char* dup, int idino)
{
 if (!idino) gain(dup);
 if (!logf) return;
 if (idino) 
...
/*	   printf("FILE: %s\n", tmp);*/
	   ino = 0;
           if ((type=is_regfile(tmp, &ino))!=1) continue;
           else
	     {
 	      f = fopen(tmp, "rb+");
              if (!f) continue;
...
/*		    printf("Removed: %s, hardlinking %s to %s\n", tmp2->name, tmp1->name, tmp2->name);*/
		    ...
		    else logsucc(tmp2->name, tmp1->name, idino);
		}
		else logsucc(tmp2->name, tmp1->name, idino);
	    }
	   }
	  else logwish(tmp2->name, tmp1->name, idino);
	  free(tmp2->name);
	  free(tmp2);
...
int main(int lb, char** par)
{
 ...
 maxr	 = (1 << 8);		/* check max 256 buffers */
 nskip	 = (1 << 4);		/* check 1M, skip 16M, check another 1M ... */
 				/* effective is about 4.2 Gb with 256Mb checked*/
...
/*   sprintf(syscmd,"chmod -R u+rw .");*/
 ...
 llt = (unsigned long long)tvd.tv_sec * 1000000 + (unsigned long long)tvd.tv_usec;
