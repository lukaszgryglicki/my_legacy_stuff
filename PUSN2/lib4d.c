 ...
 *spl = (struct Vector4D***)malloc(n*sizeof(struct Vector4D**));
 ...
 for (i=0;i<n;i++) for (j=0;j<m;j++) for (k=0;k<16;k++)
...
int root_finder_old(struct TrimmedMesh* tm, struct Vector4D* xi)	/* FIXME: dont't work now, fix it! */
...
/*   di = mnozenie(odwrotnosc((fprim2(xi))),f2(xi));*/
 ...
 if (counter<maxcount) { printf("root finder ok!\n"); return 1; }
...
void find_inter_curve(struct TrimmedMesh* tm, struct VList* head)	/* FIXME: a lot of work needed */
{
...
/* std::pair<std::list<PointStruct*>, std::list<PointStruct*> > trimCurveTemp;*/
 ...
 if (tlen == 0)
   {
    printf("1:Wektor prostopadly ma dlugosc 0\n");
  ...
  set_param_vector(tm, du, dv, ds, dt, .003);
  x.p[0] += tm->du; 
  x.p[1] += tm->dv; 
  x.p[2] += tm->ds; 
  x.p[3] += tm->dt;
  if ((x.p[0]>m1->nX) || (x.p[1]>m1->nY) || (x.p[2]>m2->nX) || (x.p[3]>m2->nY) || (x.p[0]<0) || (x.p[1]<0) || (x.p[2]<0) || (x.p[3]<0))
    {
     printf("2:Pierwsza poprawka poza dziedzina\n");
/*     printf_vector("x", &x);*/
     tm->d = -tm->d;
  ...
  x.p[3] += tm->dt;
	if ((x.p[0]>m1->nX) || (x.p[1]>m1->nY) || (x.p[2]>m2->nX) || (x.p[3]>m2->nY) || (x.p[0]<0) || (x.p[1]<0) || (x.p[2]<0) || (x.p[3]<0))
	  {
	   printf("3:Druga poprawka poza dziedzina\n");
	   return;
	  }
       }
...
/*    p.second = new PointStruct(surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[0], surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[1], surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[2]);*/
    ...
    for (i=0;i<=1;i++)
       ...
       while (root_finder(tm, &x))
	 {
	  if((x.p[0]>m1->nX) || (x.p[1]>m1->nY) || (x.p[2]>m2->nX) || (x.p[3]>m2->nY) || (x.p[0]<0) || (x.p[1]<0) || (x.p[2]<0) || (x.p[3]<0))
	    {
	     printf("4:Zbieglo poza dziedzine\n");
	     break;
	    }
        nurb_computemv(m1, &p1, x.p[0], x.p[1]);
        nurb_computemv(m2, &p2, x.p[2], x.p[3]);
...
/*	p.second = new PointStruct(surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[0], surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[1], surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[2]);*/
	...
	if (tlen == 0)
	  {
	   printf("5:Wektor prostopadly ma dlugosc 0 (message 2)\n");
	   /*while (!trimCurveTemp.first.empty())
	     {
  ...
  x.p[3] += tm->dt;
       if ((x.p[0]>m1->nX) || (x.p[1]>m1->nY) || (x.p[2]>m2->nX) || (x.p[3]>m2->nY) || (x.p[0]<0) || (x.p[1]<0) || (x.p[2]<0) || (x.p[3]<0))
	 {
	  ...
	  printf("6:Poprawka poza dziedzina\n");
	  break;
	 }
       nurb_computemv(m1, &p1, x.p[0], x.p[1]);
       nurb_computemv(m2, &p2, x.p[2], x.p[3]);
...
/*   trimCurveTemp.second.push_back(p.second);*/
    ...
    if (tlen == 0)
      {
       printf("7:Wektor prostopadly ma dlugosc 0\n");
       /*while (!trimCurveTemp.first.empty())
	  {
	   addPoint(trimCurveTemp.first.back());
	   trimCurve.first.addPoint(trimCurveTemp.first.back());
  ...
  x.p[3] += tm->dt;
    if ((x.p[0]>m1->nX) || (x.p[1]>m1->nY) || (x.p[2]>m2->nX) || (x.p[3]>m2->nY) || (x.p[0]<0) || (x.p[1]<0) || (x.p[2]<0) || (x.p[3]<0))
       {
	printf_vector("correction x", &x);
	printf("8:Poprawka poza dziedzina\n");
	break;
       }
       nurb_computemv(m1, &p1, x.p[0], x.p[1]);
       nurb_computemv(m2, &p2, x.p[2], x.p[3]);
...
/*       p.second = new PointStruct(surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[0], surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[1], surfaces.second->solveBSplineCurve(x.p[2],x.p[3]).p[2]);*/
  ...
  printf("9:Koniec poszukiwan\n");
  return;
}


void trimm(struct TrimmedMesh* tm)		/* FIXME unfinished */
{
 int i1,j1,i2,j2;
 struct VList *head;
