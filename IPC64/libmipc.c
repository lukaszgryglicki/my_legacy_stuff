...
int msemidfromkey(key_t semkey)
{
 ...
 if (ret==-1) 
   {
     warn("semget in msemidfromkey, key=%d", semkey);
     perror("semget");
   }
 return ret;
}
...
int msemget(key_t semk)
{
 ...
 if (semid==-1) 
   {
    warn("semaphore don't exists, msemget:key=%d", semk);
    perror("semget");
   }
 return semid;
}
...
int mdropsem(int semid)
{
 if (semctl(semid,IPC_RMID,NULL)==-1) 
   { 
    warn("mdropsem failed for semid %d", semid); 
    perror("semctl");
    return -1; 
   }
 else return 0;
...
{
 int semid;
 if ((semid=semget(semkey,1,0660))==-1) 
   { 
    warn("semget in mdropsemkey"); 
    perror("semget");
    return -1; 
   }
 if (semctl(semid,IPC_RMID,NULL)==-1) 
   { 
    warn("semctl in mdropsemkey"); 
    perror("semctl");
    return -1; 
   }
 else return 0;
...
{
 int msgid;
 if ((msgid=msgget(msgkey,0660))==-1) 
   { 
    warn("msgget in mdropmsgkey"); 
    perror("msgget");
    return -1; 
   }
 if (msgctl(msgid,IPC_RMID,NULL)==-1) 
   { 
    warn("msgctl in mdropmsgkey"); 
    perror("msgctl");
    return -1; 
   }
 else return 0;
...
{							/* MEM FOR TEXT MUST BE ALLOCATED, MSGSIZ+1 bytes */
 ...
 struct mmsg msg;
 if (wt)		/* IF WAIT SET */
   {			/* xXXx should be type there instead of 0 */
    if ((len=msgrcv(msgid,&msg,MSGSIZ,0,MSG_NOERROR))==-1) { perror("msgrcv in mmsgrecvtxt"); return -1; }
  ...
  {
   			/* xXXx should be type there instead of 0 */
    if ((len=msgrcv(msgid,&msg,MSGSIZ,0,MSG_NOERROR|IPC_NOWAIT))==-1)
    {
...
int mmsgrecvstr(int msgid, struct mmsg* msg, int wt)	/* MEM FOR STRUCT MUST BE ALLOCATED */
{
 ...
 if (wt)		/* IF WAIT SET */
   {
   			/* xXXx should be type there instead of 0 */
    if ((len=msgrcv(msgid,&msg,MSGSIZ,0,MSG_NOERROR))==-1) { perror("msgrcv in mmsgrecvstr"); return -1; }
...
int mmsgrecptxt(int msgid, char* txt, int wt)	/* 'recp' uses priority and fetch with minimal */
{
 ...
 if (wt)		/* IF WAIT SET */
   {				/* WHY THERE IS ZERO ?? */
	   			/* FIXME should be -PRIOR xXXx*/
    if ((len=msgrcv(msgid,&msg,MSGSIZ,(0),MSG_NOERROR))==-1) { perror("msgrcv in mmsgrecptxt"); return -1; }
...
int mmsgrecpstr(int msgid, struct mmsg* msg, int wt)	/* MEM FOR STRUCT MUST BE ALLOCATED */
{
 ...
 if (wt)		/* IF WAIT SET */
   {
	   			/* xXXx 0 here?? */
    if ((len=msgrcv(msgid,&msg,MSGSIZ,(0),MSG_NOERROR))==-1) { perror("msgrcv in mmsgrecpstr"); return -1; }
    /* needn't set 0 here, because struct can hold binary data */
    return 0;
   }
 else			/* RETURN IMMADIATELY */
...
int mmsgidfromkey(key_t msgkey)
{
 ...
 if (ret==-1) 
 {
	 warn("msgget in mmsgidfromkey, key=%d", msgkey);
	 perror("msgget");
 }
 return ret;
}
...
{
 int shmid;
 if ((shmid=shmget(shmkey,1,0660))==-1) 
 { 
 	warn("shmget in mdropshmkey"); 
	perror("shmget");
 	return -1; 
 }
 if (shmctl(shmid,IPC_RMID,NULL)==-1)   
 { 
 	warn("shmctl in mdropshmkey"); 
	perror("shmctl");
	return -1; 
}
 else return 0;
...
int mshmidfromkey(key_t shmkey)		/* SIZE ISN'T KNOW !! */
{
 ...
 if (ret==-1) 
 {
	 warn("shmget in mshmidfromkey, key=%d", shmkey);
	 perror("shmget");
 }
 return ret;
}
